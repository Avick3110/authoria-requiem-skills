# Consumables — Integration & Crafting Corpus (ALCH / INGR / COBJ)

*Read-only mining of the live ARR 2.0 load order via houseCARL. Instance: `E:\Skyrim Modding\ARR 2.0`,
profile "Authoria - Requiem Reforged - NSFW Profile". Feeds a new Requiem-patching skill for consumables.
All FormIDs are verbatim wire tokens `XXXXXX:Plugin.esp` — reusable in a write.*

---

## 0. Headline doctrine fact — the Reqtificator does NOT touch consumables
...
Query evidence:
- `cross_plugin_query type=ALCH plugins=["Requiem for the Indifferent.esp"] group_by=defined_in` → **0 matches**
- `cross_plugin_query type=INGR plugins=["Requiem for the Indifferent.esp"] group_by=defined_in` → **0 matches**
- `plugins=["Requiem for the Indifferent.esp"] defined_in=true group_by=type` → only Npc + LeveledNpc.

RftI touches **zero** ALCH, INGR, and COBJ — not as a definer and not as an override (the ALCH/INGR
`plugins=[RftI]` scans return 0 for *touches*, not just for *defines*). **Consumables are entirely a
hand-authored ESP domain.** Unlike weapons/armor/NPCs — where you carry inputs and let the Reqtificator
emit the final balanced record — for a potion, food, drink, ingredient, oil, or poison **the ESP override
...
| Authoria - Reqtificated - Unslaad / Glenmoril / Remiel / LOTD / Morskom / Mrissi … | 13/11/10/9/6/6 | per-mod content patches |
...
often modded than potions/food — the team mostly leans on AR's own ingredient set.

**The team's naming convention for a modded-consumable patch** (mirrors the weapons domain):
- `Authoria - Reqtificated - <ModName>.esp` — per content mod (Xelzaz, Vigilant, Unslaad, Sweets and Such, …)
- `Authoria - CC <Name> Reqtificated.esp` — per Creation-Club creation
- `Authoria - Master Patch - Food and Drink.esp` / `Authoria - Patch - Alcohol Tweaks.esp` / `ATweaks.esp` —
...
last); prior layers (base mod → Taberu animation patch → FaB per-mod patch) are all forwarded.

### 2a. POTION kit — keep the effect links, rebalance the shell

Content mod: `BPUFXelzazFollower.esp` (Xelzaz follower). Patch: `Authoria - Reqtificated - Xelzaz.esp`.

**`0E46A5:BPUFXelzazFollower.esp` — BPUFXelzazInvisPotion "Potion of Imperception"**
- Effects list **unchanged** between base and winner: `[0]` BaseEffect `03EB3D:Skyrim.esm` (→ **REQ_Alch_Invisibillity**,
  Invisibility, dur 20), `[1]` `0EE8D2:BPUFXelzazFollower.esp` (BPUFMufflePotion, mag 1 dur 20).
- **Only** delta the patch makes: `Value 250 → 70`. Flags = `NoAutoCalc, Medicine`.
...
**`0E97AF:...` BPUFXelzazCureDisease** — base→winner: `Flags Medicine → NoAutoCalc,Medicine`; `Value 79 → 50`;
effect `[0]` BaseEffect `0AE722:Skyrim.esm` kept, **magnitude 100 → 0** (the REQ MGEF is fixed-behaviour, so
the per-item magnitude is zeroed).
**`3C301E:...` BPUFXelzazNightEye** — `Flags Medicine → NoAutoCalc,Medicine`; `Value 0 → 60`; effect `[0]`
...
> retunes (Invisibility, CureDisease, NightEye, Waterwalking, Ethereal, Frost/Fire shields, …), **do not
> rebuild the effect list.** Set `NoAutoCalc` (+`Medicine`), assign a Requiem-consistent gold `Value`, and
> zero any magnitude the REQ MGEF ignores. Keyword set is minimal — Invis potion carries only
> `VendorNoSale = 0FF9FB:Skyrim.esm`.

### 2b. FOOD kit — rebuild the effect list to the REQ_Apo_Food template

Content mod: `KvSweets.esp` (Sweets and Such). Patch: `Authoria - Reqtificated - Sweets and Such.esp`.

**`0013DB:KvSweets.esp` — KvSweetsCakeChocolate "Chocolate Cake"** (base 2 vanilla effects → winner 7):
...
Also `Value 25 → 30`. Keywords `[VendorItemFood 08CDEA:Skyrim.esm, GiftChildSpecial 0A248C:Skyrim.esm]`
(the sweet stays child-giftable). Flags `NoAutoCalc, FoodItem`; ConsumeSound `0CAF94:Skyrim.esm` (ITMFoodEat).

**`00140B:KvSweets.esp` — KvSweetsCookieChocolate** — base 2 → winner 5 effects (Fortify Health `000002:FaB` 7,
`000B4A:FaB` 10, + nutrition + Survival hunger + Taberu); `Value 5 → 7`, `Weight 0.1 → 0.25`.

> **Food rule:** replace the mod's vanilla RestoreHealth-style effects with the REQ food kit —
...
### 2c. ALCOHOL / DRINK kit — the REQ_Alcohol template

**`FA82FA:BPUFXelzazFollower.esp` — BPUFXelzazMeadSnowberry "Xelzaz's Southfringe Mead"** (base 1 → winner 5):
...
**`776C69:...` BPUFXelzazWarmTea** — 4-effect winner including `002EE5:Update.esm` (a Survival warm/food hook)
+ `000A36:FaB` REQ_Apo_FoodNutritionSM (5) + a warm effect + Taberu marker; `Value 15 → 10`. Conflict chain is
4-deep: base → `Taberu Animation - Xelzaz.esp` → `Requiem - Food and Beverages Redone - Xelzaz.esp` →
...
`cross_plugin_query type=COBJ plugins=["Requiem - Alchemy Redone.esp"] defined_in=true` → **25** records.
...
1. **Weapon-coating oils** `REQ_Smelting_Oil_{Fire|Frost|Shock|Silver}{1,2,3}` → create `REQ_Oil_*`
2. **Weapon-coating powders** `REQ_Smelting_Powder_{Fire|Frost|Shock|Silver}{1,2,3}` → create `REQ_Powder_*`
3. **Fortify-Enchanting potions** `REQ_Cooking_Potion_FortifyEnchanting{1,2,3,4}` → create the `REQ_Potion_FortifyEnchanting*`
   (Diluted/Faint/Fair/Good) at `039CFB/039D02/039D0A/039D12:Skyrim.esm`

**Every one uses the same station:** `WorkbenchKeyword = 0A5CB3:Skyrim.esm` (**CraftingCookpot**) — NOT the
alchemy lab. Requiem routes these alchemical crafts through the cooking pot.

**Recipe shape (verbatim):**
- `REQ_Smelting_Oil_Fire2` (`000830`): Items = `03AD5E:Skyrim.esm` FireSalts ×1 + `0F11C0:Skyrim.esm`
  DwarvenOil ×3 → `CreatedObject 000807:AR` (REQ_Oil_Fire) ×**9**. (Tier 1 makes ×6; count scales with tier.)
  Conditions: `HasPerk 0C07CA:Skyrim.esm` (**REQ_Alchemy_AlchemicalLore2**) `== 1`; `GetActorValue Alchemy >= 50`;
  `GetActorValue Alchemy < 100` — i.e. a **skill-band-gated tier**.
- `REQ_Cooking_Potion_FortifyEnchanting2` (`000821`): Items = `063B5F:Skyrim.esm` SprigganSap ×1 +
  `06B689:Skyrim.esm` HagravenClaw ×1 → REQ_Potion_FortifyEnchanting2 ×1. Conditions:
  `HasKeyword AD3A3B:Requiem.esp` (**REQ_RacialSkills_CreatePotionsUnperked**, flagged **OR**) — OR —
...
### 3b. Cooked-food recipes — a separate, thin, ungated Authoria layer

Reverse-lookup `type=COBJ references=[FaB food FormIDs]`:
- `000D58:FaB` REQ_Food_Baked_GarlicBreadSlice → **no COBJ** (loot/vendor only; not craftable).
- `000C53:FaB` REQ_Food_Meat_BearRaw → **one** COBJ: `000807:Authoria - Patch - Food Recipes.esp`
  `Authoria_Cook_Food_Cooked_BearMeat`: Items = `000C53:FaB` BearRaw ×1 → `000801:Authoria - Patch - Food
  Recipes.esp` (Authoria_Food_Cooked_BearMeat "Bear Haunch") ×1, station CraftingCookpot, **Conditions = []
  (ungated)**.

`Authoria - Patch - Food Recipes.esp` = **6 COBJ + 6 Ingestible** — a small custom layer turning FaB raw
meats into cooked foods, with **no perk/skill gate**. This is distinct from the modded-consumable *patch*
pattern (§2) and from AR's perk-gated alchemy COBJ (§3a). **FaB itself defines no cooking recipes** — most FaB
...
The Requiem addon satellites are **thin, upstream** overrides — the model of "how a proper compat patch for
FaB/AR looks" — but note the Authoria "Reqtificated" patch usually wins *on top* of them.

- **`Requiem - Food and Beverages Redone - LoTD.esp`** = **1 Ingestible**, `2E0517:LegacyoftheDragonborn.esm`
  DBM_cinnaspicecookie. Base→winner: `Value 4 → 10`, `Weight 0.5 → 0.75`, vanilla effects swapped to the REQ
  kit (`18ED7C:Requiem.esp` etc.), **+1 FaB food keyword `000A21:FaB`**. Clean 2-plugin chain (base → FaB-LoTD
  winner). **This is the canonical satellite shape: retarget effects → REQ kit, add FaB nutrition/food keyword,
  rebalance value/weight.**
- **`Requiem - Food and Beverages Redone - Auri.esp`** = **1 Ingestible**, Jagga (`30E7E8:018Auri.esp`). The
  satellite renames the EditorID to `REQ_Alcohol_Moderate_Jagga`. 5-deep chain: `018Auri.esp` → `Snazzy_Auri_Items.esp`
  → `...FaB-Auri.esp` → `DBM_Auri_Patch.esp` → **`Authoria - Reqtificated - Auri.esp` (winner)**. Winner uses the
  REQ_Alcohol kit (`18ED7C` FortifyHealth 45, `AE3729` Inebriation 15, + nutrition + Survival), `Value 5 → 10`,
  `Weight 0.5 → 0.75`, adds FaB keyword `000C51:FaB`.
- **`Requiem - WAR Alchemy Redone Patch.esp`** = **1 Quest**, `000A9A:Requiem - Weapons and Armor Redone.esp`
  ...
  script property on the alias). **Not a consumable patch** — it bridges WAR throwing knives into Requiem's
  poison-application system. Belongs to the alchemy *systems* boundary, not the item-balancing story.

---

## 5. LVLI boundary (characterize only — placement doctrine is another skill)
...
- **AR touches 106 LeveledItem**: defined_in Requiem.esp 55, Skyrim.esm 37, Dragonborn.esm 6,
  **Requiem - Alchemy Redone.esp 6**, Dawnguard.esm 2. AR **defines 6** of its own: `LItemPotionFortifyUnarmed(Best)`,
  ...
  16 entries, flags `CalculateFromAllLevelsLessThanOrEqualPlayer, CalculateForEachItemInCount` — a tiered pool of
...
- **FaB touches 20 LeveledItem**: Skyrim.esm 19 + Requiem.esp 1 — FaB **edits vanilla food/loot lists** to
...
- **GMST `fAlchemySkillFactor` (`10C4D9:Skyrim.esm`)**: vanilla 1.5 → Requiem.esp 1.1 → **AR 2** (winner). AR
  *raises* the alchemy skill→potency scaling (potions get stronger per skill point). Single knob, whole-system.
- **AVIF `AVAlchemy` (`000456:Skyrim.esm`)**: AR touches it (override depth 5) but the live winner is
  `Requiem - AR Special Feats Patch.esp` — the Alchemy skill's actor-value definition (skill-use description /
...
- **8 Perk overrides** (AR retunes the Alchemy perk tree): `REQ_Alchemy_ImprovedElixirs 058216`,
  ...
  potion strength multipliers, poison behaviour, and the crafting-condition perks referenced by the COBJ in §3a.
- **No GLOB** touched by AR (`type=GLOB plugins=[AR]` → 0).
...
1. **EditorID scheme divergence on the same item.** FaB-Auri renames Jagga → `REQ_Alcohol_Moderate_Jagga`,
...
2. **Potion vs food effect-handling asymmetry.** Potions **keep** their (vanilla-FormID) effect links and only
   rebalance Value/Flags/magnitude — Requiem's MGEF override carries the behaviour. Food/drink get their entire
   effect **list rebuilt** to the `REQ_Apo_Food*` / `REQ_Alcohol_*` kit. Same domain, opposite technique — the
...
3. **"Alchemy" crafts happen at the cooking pot.** AR's oils, powders, and fortify-enchanting potions all use
   `CraftingCookpot 0A5CB3:Skyrim.esm`, gated by Alchemy perks/skill — not the alchemy lab. Counter-intuitive;
...
4. **Two unrelated crafting layers with different gating philosophies.** Requiem's own consumable COBJ are
   perk+skill-band gated (§3a); the team's `Authoria - Patch - Food Recipes.esp` cooked-food COBJ are **ungated**
...
| AR COBJ station | CraftingCookpot (0A5CB3) — 100% of sampled | batch read |
