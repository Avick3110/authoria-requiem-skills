...
"Authoria - Requiem Reforged - NSFW Profile", instance `E:\Skyrim Modding\ARR 2.0`, 3558
active plugins). Empirical base for the planned `requiem-perk-assignment` skill — a skill that
...
- A patch that wants an NPC to fight like a Requiem actor **assigns vanilla Skyrim FormIDs**
  (`0BABE4:Skyrim.esm` = Armsman) — the live conflict winner is Requiem's re-authored version
...
- Requiem source NPCs carry a **tiny hand-picked set of player-tree perks** (4–5). The Reqtificator
...
- The skill's write set = **player-tree perks only**. The `RFTI_All_*` / `RFTI_Player_*` /
  `Nox_Perk_Mechanics_*` machine perks are **forbidden output** — the Reqtificator owns them.

---

## 1. The perk space — enumerated totals
...
The 367 Skyrim.esm + 67 DLC overrides are **the assignable player trees** — a patch references the
**vanilla** FormID and the live winner delivers Requiem's edit (see §2a).
...
17 plugins — 56 still win in Requiem.esp, 28 in the Resist-and-Regen-tweak patch, 28 in Magic Redone,
23 in WAR, 9 in Races Redone, etc. **The origin FormID is `…:Requiem.esp` regardless of who wins** —
...
| **Requiem - Weapons and Armor Redone.esp (WAR)** | **28** | weapon-specialization tricks (Disarming Blade, Armsbreaker…), `RFTI_All_WAR_Mechanics`, and **`RFTI_Player_UnarmedMechanics` (000A84)** + `REQ_OneHanded_HandToHand` unarmed line — the known "HandToHand REQ perk lives in a WAR plugin" fact, **confirmed** |
...
**mod-shipped** perks (see §3). Largest non-Requiem definers: Apocalypse (57), LotD (56), Subclasses
...
### 2a. PLAYER-TREE perks — the source-carried, assignable set

These are the perk-tree nodes. **A multi-rank line is NOT one FormID at Rank N — it is a chain of
...
**Vanilla→Requiem delta proof** — `0BABE4:Skyrim.esm` read with conflict_tree:

- Vanilla: EditorID `Armsman00`, Name "Armsman", "One-Handed weapons do 20% more damage",
  Effect `ModAttackDamage ×1.2`.
- **Live winner** (Requiem - Weapons and Armor Redone.esp, override_depth 4): EditorID
  `REQ_OneHanded_WeaponMastery1`, Name "Weapon Mastery", "…swing one-handed weapons or fists…
  [25% more damage]", Effect `ModAttackDamage ×1.25`, `NextPerk=079343:Skyrim.esm`, `NumRanks=1`.
- 4 plugins touch it (Skyrim.esm → Requiem.esp ×1.2 → Minor Arcana Roleplaying → WAR ×1.25 winner).
- **This is the mechanism:** assign `0BABE4:Skyrim.esm`, the actor gets WAR's 25% Weapon Mastery.

Sample of the assignable player-tree space (all confirmed live, FormID → live winner editorid):
...
### 2b. REQTIFICATOR-ASSIGNED mechanics perks — FORBIDDEN output
...
| AD394B | RFTI_All_ArmorPenetration_PowerAttacks | "GM: … Power Attack Penetration" |
...
| 962798 / 962799 | RFTI_All_PoisonRescaling / RFTI_All_AbsorbRescaling | "REQ GM: Poison/Absorb … Rescaling" |
| 6B9709 / 703B25 / 95FFFB / 755649 | RFTI_All_Stress_AttackStaminaCost / ArmoredCasting / ExhaustionPenalties / MassEffect | "REQ GM: …" stress line |
...
/ `…PowerModifier` (Skyrim.esm 0CF788 / 0A725C).

#### The boundary before/after proof — REQ_Bandit_Template_SwordShield_01 (`86837E:Requiem.esp`)
...
- **Live winner** (`plugin="Requiem for the Indifferent.esp"` = Reqtificator output): **50 perks** —
...
**Precise boundary: source 5 → build 50.** The skill writes into the 5-perk source lane; the +45 are
the Reqtificator's exclusive stamp and must never appear in a hand-authored patch. Second NPC checked
(`879914 REQ_Bandit_Template_Bow_01`, source): 4 perks — WeaponMastery1, RangedCombatTraining,
Improved Blocking, Agility — same pattern.

### 2c. PLAYER-EXCLUSIVE skill perks — exist, never on an NPC

The `RFTI_Player_*` skill perks are the player's control-center perks. Verified they resolve and are
**absent from every Requiem source NPC**: a reverse lookup —
`type=NPC_ defined_in=[Requiem.esp] references=[all 8 exclusive FormIDs]` — returned **0 matches**.
...
- On an NPC, each rank is its **own `PerkPlacement` entry** with `Perk=<that rank's FormID>` and
  ...
  as two separate entries, both Rank=1 — **not** one FormID at Rank 2.
...
- `type=PERK plugins=[Requiem.esp]` group_by=defined_in → **599** across 7 groups (162 Requiem-defined).
- `type=PERK plugins=[Requiem.esp] defined_in` group_by=winner → 162 across 17 winner plugins.
- `type=PERK plugins=[Requiem - Magic Redone.esp] defined_in` → **50**.
- `type=PERK plugins=[Requiem - Weapons and Armor Redone.esp] defined_in` → **28** (incl. HandToHand line).
- `type=PERK` group_by=defined_in (whole order) → **1876** across **168** plugins.
- Reverse lookup `type=NPC_ defined_in=[Requiem.esp] references=[8 player-exclusive perks]` → **0**.
- Enumerated 148 of 162 Requiem-defined perk editorids (limit truncation at 200 returned 148 lines;
  the classification pattern — REQ_\* tree / RFTI_Player_\* / RFTI_All_\* / RFTI_Trait_\* — holds
...
- The Reqtificator stamps all 10 `RFTI_All_PlayableRace_*` and the full `RFTI_Ench_*` artifact set on
  ...
  semantics — not exhaustively swept).
- The disposition rule (§3) is calibrated from 3 sampled mods; **inferred** to generalize by prefix
