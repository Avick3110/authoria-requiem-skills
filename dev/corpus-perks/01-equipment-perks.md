# Requiem Corpus — Equipment → Combat-Perk Assignment

*Mined live through houseCARL against the ARR 2.0 Requiem load order (profile
"Authoria - Requiem Reforged - NSFW Profile", instance `E:\Skyrim Modding\ARR 2.0`).
...
## 0. The single most important finding — source vs. live winner
...
winner's `Perks` list is **~70–80 % Reqtificator boilerplate** that has *nothing* to do with the
actor's equipment. Example — `EncForsworn01TemplateMelee` (`023AA9:Skyrim.esm`): live winner = **56
perks**, of which only the **first 11** are the equipment-derived combat perks; items `[11..55]` are
universal injected infrastructure.

The boilerplate block (identifiable by prefix — **never hand-author any of these**; the Reqtificator
re-adds them at build time):

- `RFTI_All_*` — armor-penetration, resistances, stress/exhaustion, arrow recovery, armor weight,
  `RFTI_All_PlayableRace_<Race>`, `RFTI_All_ActorValueModifier`/`PowerModifier`
- `RFTI_Ench_*` — unique-artifact enchantment handlers (Wuuthrad, Ghostblade, Nettlebane, …)
- `Nox_Perk_Mechanics_*` (Magic Redone: Physical/Destruction/Conjuration/…)
...
and discard the `RFTI_/Nox_/…GM_` tail. All examples in this doc use source-carried perks.
...
1. **`REQ_Bandit_Template_<Weapon>_<Base|01..06>`** — Requiem's **purpose-built, weapon-explicit
   ...
   This is the canonical equipment→perk source. Each carries a `REQ_Class_Bandit_<Weapon>` class and a
   `REQ_Outfit_BanditArmor_<Heavy|Light>_Rank<n>[_Shield]` outfit.
2. **`REQ_LookTemplate_Enc<Family><NN><Role><Race><Sex>`** — appearance + base-combat templates for
   the vanilla encounter roster (Bandit/Forsworn/Thalmor/Guard/…). `Role ∈ {Melee1H, Melee, Missile,
   Magic, …}`.
3. **`REQ_NULL_Enc<Family><NN><Role><Race><Sex>`** — the null-stripped **leaf actors** that spawn in
   the world; each `Template`s a look-template and carries its own `Perks`+`Class`+`Outfit`.

Weapon-role **classes** (`CLAS`, all `Requiem.esp` unless noted) — the definitive weapon taxonomy:
...
## 2. The equipment → perk-family mapping (the core table)
...
| **Mace+shield** | One-Handed (MaceFocus) **[INFERRED — class read, perks not read]** | Block + Heavy Armor | Heavy | class only |
...
| **Dual-wield 1H** | One-Handed (Sword+Mace+WarAxe focus, +Flurry) | **Evasion (+Lethality)** — no Block, no Heavy | Light | DualWield_06 `000B4C:…WAR.esp` |
...
1. **Weapon line follows the weapon.** 1H → `REQ_OneHanded_*`; 2H → `REQ_TwoHanded_*`; bow/crossbow →
   `REQ_Marksmanship_*`. Within a class the **weapon-Focus family swaps by weapon and nothing else
   swaps**: SwordFocus↔MaceFocus↔WarAxeFocus (1H), GreatswordFocus↔WarhammerFocus↔BattleaxeFocus (2H).
   Verified by reading AxeShield vs SwordShield and Battleaxe vs Greatsword vs Warhammer — the
   mastery/strikes/defence shell is identical, only the Focus line differs.
2. **Shield OR 2H weapon ⇒ Block tree.** Requiem's Block skill covers weapon-blocking, so 2H heavy
   hitters get the full Block line (ImprovedBlocking→PowerfulBashes→StrongGrip→OverpoweringBashes→
   ExperiencedBlocking) even with no shield.
3. **Armor class picks the survival tree.** Heavy → `REQ_HeavyArmor_*` (Conditioning→Fortitude→
   CombatTraining→RelentlessOnslaught[→PowerOfTheCombatant]); Light/no-shield → `REQ_Evasion_*`
...
6. **Everyone melee gets** `REQ_OneHanded_WeaponMastery1/2` (`0BABE4`/`079343`) and
   `REQ_OneHanded_HandToHand` (`0AD7A3:Requiem.esp`) as the universal floor — even 2H and archer
   actors carry WeaponMastery.
7. **Pure-vanilla `Skyrim.esm` perks do not appear.** Every combat perk is a Requiem perk. The FormID
   is often `…:Skyrim.esm` (Requiem overrides the vanilla perk record **in place**) but the EditorID is
...
**contains** the previous — perks are added, never removed. `_Base` is a **skeleton** (Class + null
outfit, **Perks absent**); the numbered tiers carry the perks.

| Tier | FormID | Level | # perks | Perks added at this rung (cumulative) |
|---|---|---|---|---|
| Base | 86837D | 1 | 0 | *(skeleton — Class=SwordShield, no perks, null outfit)* |
...
RangedCombatTraining, ImprovedBlocking, Agility) → **Bow_06** (`89ED40`, Lvl 24, **17 perks**: full
Marksmanship + Evasion). 

**Escalation rules:**
- **Perk count grows ~ linearly with tier** (Bandit: ~5 → ~18 across tiers 01→06).
- **Focus/mastery ranks deepen with tier:** SwordFocus1 (t3) → +Focus2 (t6); axe/warhammer/battleaxe
  reach **Focus rank 3** by t6. WeaponMastery1+2 present from t1.
- **Defence trees fill in with tier:** Block/HeavyArmor start at one perk (t1) and add 2–3 more nodes
  by t6.
- **Level bands are family-specific, tier shape is universal.** Bandits: t1≈Lv3 … t6≈Lv24. Forsworn:
...
**Forsworn** (`EncClassForsworn`, light/unarmored shaman-warriors) are hybrids — they carry the 1H
...
- `EncForsworn__TemplateMelee` (source, cumulative): 1H (SwordFocus+WarAxeFocus **both**, since the
  ...
  (ImprovedHealing, Respite, FocusedMind) + **Alteration** (MagicResistance1→3) + ImprovedBlocking.
  Ladder: 01=11 perks (Lv7) → 03=9 → 04=14 → 05=16 → 06=20 (Lv46). Also gains bound-weapon / conjure
  spells and rising `REQ_Trait_Tempering_*` armor-tempering effects up the ladder
  (Bandit_Heavy_Rank4 → Guard_Captain → Guard_Elite).
...
- `EncForsworn06TemplateMagic` (source, 11 perks): **no weapon focus at all** — pure
...
tier)**, exactly as a warrior's are derived from its weapon — same live-analogy method, different axis.

---

## 5. Perk reference (EditorID → FormID, by tree)

*All `…:Skyrim.esm` unless noted; all are Requiem-overridden perks (winner Requiem.esp / WAR.esp).*

**One-Handed** — WeaponMastery1 `0BABE4`, WeaponMastery2 `079343`; SwordFocus1/2/3 `05F56F`/`0C1E90`/
...
**Two-Handed** — GreatWeaponMastery1/2 `0BABE8`/`079346`; GreatswordFocus1/2/3 `03AF83`/`0C1E94`/
...
**Block** — ImprovedBlocking `0BCCAE`, PowerfulBashes `058F67`, StrongGrip `058F68`, ElementalProtection
`058F69`, OverpoweringBashes `05F594`, ExperiencedBlocking `079355`.

**Heavy Armor** — Conditioning `0BCD2A`, Fortitude `058F6C`, CombatTraining `058F6F`, RelentlessOnslaught
`07935E`, PowerOfTheCombatant `107832`.

**Evasion (light)** — Agility `0BE123`, Finesse `051B1B`, Dexterity `051B1C`, CombatReflexes `051B17`,
Dodge `079376`, WindWalker `105F22`, Lethality `18F5A8:Requiem.esp`.

**Caster schools (hybrid/mage)** — Destruction: Pyromancy1/2 `0581E7`/`10FCF8`, Electromancy1/2
...
- **Read Base + first + middle + last of the SwordShield ladder** (Base/01/03/06): monotonic-additive,
  no removals, no reordering surprises. Base carries **zero** perks (skeleton) — do **not** assume the
...
- **Cross-weapon reads (SwordShield vs AxeShield; Greatsword vs Warhammer vs Battleaxe):** the shell is
  shared, the Focus line swaps — **but minor per-weapon variation exists** and must not be extrapolated
...
- **Outlier — vanilla "Melee2H" leaf mismatch:** `REQ_NULL_EncBandit06Melee2HNordM` (`03DE6A`) carries
  a **sword+shield** perk set and `Class=REQ_Class_Bandit_SwordShield` despite the "2H" in its
  EditorID — Requiem re-tasked the vanilla weapon-role split. **Trust the `Class` + `Outfit`, not the
...
- `EncThalmor*`: 59 (Melee1H/Melee1HShield/Melee1HDual/Missile/Magic × tier 01–06 × M/F + boss).
- `EncGuard*`: 17 (Imperial/Sons look-templates + 2 base templates).
- `REQ_NULL_EncBandit*Melee2H*`: 71 (weapon-role × race × sex × tier, incl. Berserk).
- `REQ_Bandit_Template_*` (purpose-built ladders): **84** — SwordShield/MaceShield/AxeShield,
...
- Weapon-role classes: 12 enumerated (table §1).
...
enumerated only — Thalmor confirms the Melee1H/Shield/Dual/Missile/Magic weapon-role axis by EditorID
