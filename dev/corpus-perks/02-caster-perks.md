# Corpus mining â€” Caster & hybrid perk assignment in Requiem

Empirical base for a future `requiem-perk-assignment` skill. Mined live through houseCARL against
the ARR 2.0 instance (`Authoria - Requiem Reforged`, 3558 plugins resolved).
...
1. **School-mastery layer â€” SOURCE-carried, hand-authored, tier-scaled.** Requiem.esp (and its
   boss records) hand-place a small list of `REQ_<School>_<SubTree>_<NNN>_<Name>` player-tree perks
   â€” the flavour/damage/utility nodes that match the actor's schools and tier. Count grows with
   tier: **0 perks at tier 1 â†’ 6 at tier 4 â†’ 8 at tier 7 â†’ 11â€“13 at boss/Dragon-Priest tier.**
   This is the ONLY perk layer a patch author authors.

2. **Generic mechanics chassis â€” Reqtificator-STAMPED at build, identical on every actor.** RftI
   ...
   author NEVER hand-places these** â€” the Reqtificator adds them, exactly as it does the generic
...
carries 45 perks, ALL of them the generic chassis (0 mastery perks â€” a tier-1 mage earns none).
...
- **The mastery-perk set expresses "tier" through the threshold suffix**, not just count:
  MagicResistance climbs `025 â†’ 075` as the fire ladder goes tier-4 â†’ tier-7.
...
Requiem **defines no new caster NPCs of its own** â€” every caster ladder is a vanilla
...
spirits/atronachs/undead, illusion decoys) â€” the *targets* of conjuration/illusion spells, not
...
set `Template â†’ EncWarlock00Template` and inherit stats/perks/spells from the tiered
`EncWarlockNN Template<School>` records. **The Template records carry the balance data** â€” read
...
- Every winner carries the RftI ActorEffect tail: `RFTI_NPC_PersistentSpellRescaling` `AD3977`,
  ...
  `RFTI_All_ExhaustionController` `062275`. Source records do NOT carry these â€” RftI adds them.
...
|---|---|---|---|
| Warlock ladder | `EncWarlock*` | **115** | The caster spine: Fire/Ice/Storm/Necro/Conjurer Ã— tiers 01â€“07, each with a Template, Boss-Template, and concrete race/gender leaf actors |
| Witch | `EncWitch*` | 15 | Fire/Ice/Storm Ã— tier 01â€“02; many leaves won by `Authoria - Master Patch - NPCs Merge.esp` |
| Vampire | `*Vampire*` | 32 | Tiered `EncVampire00â€“06Template` + `EncVampireNNTemplateBoss` + DLC1 `DLC1EncVampireTemplate{,Magic,Missile}` (hybrids) |
| Dragon Priest | `DragonPriest` | 15 | `EncDragonPriest{,Fire,Frost,Shock}` + named bosses (Morokei, Rahgot, Krosis, Otarâ€¦) |
...
`EncNecromancer`/`EncConjurer`/`EncWizard` as standalone editorids **do not exist** â€” necromancers
...
the skill level (000/025/050/075/100 â†’ Novice/Apprentice/Adept/Expert/Master) at which a *player*
...
- **Mastery gate (NPCs skip):** `_Mastery_000_Novice` `0F2CA8` Â· `_025_Apprentice` `0C44BF` Â·
  `_050_Adept` `0C44C0` Â· `_075_Expert` `0C44C1` Â· `_100_Master` `0C44C2`
- **Pyromancy (fire):** `_Pyromancy_025_Pyromancy1` `0581E7` Â· `_050_Pyromancy2` `10FCF8` Â·
  `_050_Cremation` `0F392E` Â· `_100_FireMastery` `179121`
- **Cryomancy (frost):** `_Cryomancy_025_Cyromancy1` `0581EA` Â· `_050_Cyromancy2` `10FCF9` Â·
  `_050_DeepFreeze` `0F3933` Â· `_100_FrostMastery` `179123`
- **Electromancy (shock):** `_Electromancy_025_Electromancy1` `058200` Â· `_050_Electromancy2`
  `10FCFA` Â· `_050_ElectrostaticDischarge` `0F3F0E` Â· `_100_LightningMastery` `179124`
- **Empower:** `_Empower_025_EmpoweredDestruction` `0153CF` Â· `_050_Impact` `0153D2`
...
- **Mastery gate (skip):** `000` `0F2CA7` Â· `025` `0C44BB` Â· `050` `0C44BC` Â· `075` `0C44BD` Â· `100` `0C44BE`
- **Necromancy:** `_025_Necromancy` `0581DD` Â· `_050_Ritualism` `17911B` Â· `_050_Thrallmaster`
  `0060AE` Â· `_075_DarkInfusion` `0581DE`
- **Bound weapons:** `_Bound_025_MysticBinding` `0640B3` Â· `_050_MysticInfusion` `0D799E` Â·
  `_075_MysticEmpowerment` `0D799C` Â· `_100_MysticDisruption` `17911A`
- **Daedric:** `_025_DaedricBinding` `105F30` Â· `_050_GatesOfOblivion` `0CB419` Â·
  `_050_DarkChanneling` `0060AD`... `_075_WatersOfOblivion` `0CB41A`
- **Spirit (MR):** `_025_SpiritCalling` `006048` Â· `_050_SpiritualBond`/`SpiritGuardian` Â·
  `_075_AncientSpirits` `AD385A`
- **Soul Magic (MR):** `_025_SoulMagic` `0060AD`... Â· `_075_Necropotence` `00604C`
- **Empower:** `_025_EmpoweredConjuration` `0153CE` Â· `_050_CognitiveFlexibility1` `185736` Â·
  `_100_CognitiveFlexibility2` `185737` (the empower/dual-school nodes the task asked about)

### Restoration (20 perks)
- **Mastery gate (skip):** `000` `0F2CAA` Â· `025` `0C44C7` Â· `050` `0C44C8` Â· `075` `0C44C9` Â· `100` `0C44CA`
- **Healing:** `_Healing_025_ImprovedHealing` `0581F8` Â· `_050_Respite` `0581F9`
- **Protection/Wards:** `_Protection_025_ImprovedProtection` `005FBE` Â· `_050_ImprovedWards` `068BCC`
- **Recovery:** `_Recovery_025_FocusedMind` `0581F4` Â· `_050_PowerOfLife` `0A3F64` Â·
  `_075_EssenceOfLife` `17E062`
- **Turning:** `_025_ImprovedTurning` `005FBD` Â· `_075_MysticTurning` `0581E4`
- **Specialist (MR):** Venomancy 025/075, Heliomancy 025/075
- **Empower:** `_Empower_025_EmpoweredRestoration` `0153D1`

### Alteration (22 perks)
- **Mastery gate (skip):** `000` `0F2CA6` Â· `025` `0C44B7` Â· `050` `0C44B8` Â· `075` `0C44B9` Â· `100` `0C44BA`
- **MagicResistance (tier ladder):** `_025_MagicResistance1` `053128` Â· `_050_MagicResistance2`
  `053129` Â· `_075_MagicResistance3` `05312A`
- **MageArmor:** `_025_ImprovedMageArmor` `0D7999` Â· `_050_VersatileArmor` `21792A`
  (legacy proxies `REQ_LEGACY_MageArmor50` `0D799A` / `MageArmor70` `0D799B` are what actors carry)
- **Metamagic:** `_050_Stability` `0581FC` Â· `_075_MetamagicalThesis` `21792B` Â·
...
### Illusion (19 perks)
- **Mastery gate (skip):** `000` `0F2CA9` Â· `025` `0C44C3` Â· `050` `0C44C4` Â· `075` `0C44C5` Â· `100` `0C44C6`
...
## School â†’ perk mapping, read from real actors (source/mastery layer only)
...
|---|---|---|---|---|---|
| EncWarlock01TemplateFire `044CD7` | 1 | **0** | â€” | â€” | Flames (Dest1) |
...
| EncWarlock05TemplateBossFire `0E0FCE`â€  | 45 | 8 | Pyromancy1+2 | (via ImprovedWards) | Fireball (Dest3) |
...
â€  `0E0FCE` is **won by Requiem.esp, not RftI** (a source-defined boss template RftI didn't
re-process in this order) â€” so its live winner shows the 8 raw mastery perks with **no** generic
...
- **Hagraven (`023AB0`, only 2 mastery perks):** Pyromancy2(050) + ImprovedHealing(025), despite
  level 35 â€” leans on natural weapons + Fireball/Firebolt, minimal perk investment. **Outlier: a
...
templated off `EncDragonPriestShock` yet carries **Pyromancy** perks and an Incinerate/fire kit â€”
...
winner â€” the largest read. It combines:
...
Rule for hybrids: **the melee family and the school family stack â€” neither is dropped.** The melee
...
PcLevelMult level, pure vampire-trait ActorEffects â€” a **non-caster** vampire carries zero mastery
...
## Spell-kit â†” perk correlation (confirmed)
...
highest spell tier.
- Fire01 casts only Flames (Dest1) + Steadfast Ward â†’ 0 damage perks.
- Fire04 casts up to Fireball (Dest3) â†’ Pyromancy1(025).
- Fire07 casts up to Incinerate (Dest4) â†’ Pyromancy1(025)+Pyromancy2(050).
- Necro07 casts frost (Ice Sphere/Icy Spear) AND reanimation â†’ Cryomancy1+2 AND Necromancy+DarkInfusion.

So the derivation for a NEW caster is: **read its spell schools + highest spell tier â†’ place the
matching SubTree damage node(s) at the corresponding threshold + the shared survivability set â†’
leave the rest to the Reqtificator.**

## Magicka & attribute handling

Magicka is NOT a per-record `MagickaOffset` on the NPC root â€” it lives at
**`Configuration.MagickaOffset`**, and it scales with caster tier. Fire07 (`1091A9`):
`Configuration.MagickaOffset = 200`, `HealthOffset = 100`, `StaminaOffset = 0` â€” a caster dumps its
...
`RFTI_NPC_PersistentSpellRescaling` ActorEffect, not by per-record magnitude edits â€” which is why
...
- **First/middle/last of the Warlock-Fire family** read (tier 1/4/7) â€” confirms monotonic perk-count
  and threshold climb; tier 1 genuinely carries **zero** mastery perks (not a read error â€” source is
  `(absent)`).
...
- **Hagraven outlier:** only 2 mastery perks at level 35 â€” deliberate under-investment for a
...
RftI-added â€” the isolation that produced the headline rule.
...
source-read â€” inferred from its winner + Dragon-Priest analogy), MagickaOffset tier-scaling curve
(one data point read; trend inferred), and the specialist-tree â†’ actor mapping for MR sub-trees not
